...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
            ann_file=data_root +
            'annotations_70/instancesonly_filtered_gtFine_train.json',
            img_prefix=data_root + 'leftImg16bit/train/',
            pipeline=train_pipeline)),
    val=dict(
        type=dataset_type,
        ann_file=data_root +
        'annotations_70/instancesonly_filtered_gtFine_val.json',
        img_prefix=data_root + 'leftImg16bit/val/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root +
        'annotations_70/instancesonly_filtered_gtFine_test.json',
