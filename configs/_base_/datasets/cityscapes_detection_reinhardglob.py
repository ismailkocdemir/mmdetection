...
img_norm_cfg = dict(
    mean = [ 98.87212964, 108.60482413,  99.33582091],
    std = [39.08661302, 40.47039182, 38.7278825 ],
    to_rgb=False)
train_pipeline = [
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            img_prefix=data_root + 'leftImgReinhardGlobal/train/',
            pipeline=train_pipeline)),
    val=dict(
        type=dataset_type,
        ann_file=data_root +
        'annotations_16bit/instancesonly_filtered_gtFine_val.json',
        img_prefix=data_root + 'leftImgReinhardGlobal/val/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root +
        'annotations_16bit/instancesonly_filtered_gtFine_test.json',
        img_prefix=data_root + 'leftImgReinhardGlobal/test/',
        pipeline=test_pipeline),
    reference_HDR=dict(
        type=dataset_type,
        ann_file=data_root +
